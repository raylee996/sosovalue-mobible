...
import OptLoading from "components/operation/OptLoading";
...
const ResearchDialog = () => {
  ...
  const getArticle = async () => {
    // Loading.start();
    const res = await getHotArticleDetail(router.query.id as string);
    setArticle(res.data);
  ...
  const copyLink = () => {
    copyText(`${window.location.origin}/cluster/${article?.id}`);
  ...
  const escapeNode = article && (
    <>
      {article.isAiGeneration === 1 && article.transferStatus >= 1 && (
        <div className="mt-16 flex items-center justify-center text-center text-xs text-[#ADADAD]">
          {t("Powered scene 1") && (
            <Image
  ...
  return (
    <div className="h-full bg-[#0A0A0A] w-full overflow-hidden flex flex-col items-stretch">
      ...
      <div
        className="pt-4 flex-1 h-0 overflow-y-auto overflow-x-hidden"
        ref={scrollDivRef}
      >
        <div className="px-4">
          ...
          <div className="flex items-center flex-wrap gap-2 my-4">
            ...
            {article?.matchedCurrencies &&
              JSON.parse(article.matchedCurrencies).map(
          ...
          <Accordion className="mb-3">
            ...
            <AccordionDetails className="bg-[#1A1A1A]">
              <div className="mb-4 px-3">
                {article?.informationDoVos?.map((item: any, index: number) => {
                  return (
                    <div
                      key={item.id}
                                  ...
                                  {items.sector &&
                                    ...
                                    items.sector != "null" && (
                                      <div
                                        ...
                                        className="mr-3 rounded-3xl text-xs cursor-pointer flex items-center font-bold px-2 py-0.5 text-[#D6D6D6] bg-[#333333]"
                                      ></div>
                                    )}
