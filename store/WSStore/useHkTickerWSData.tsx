...
export type HkTickerWSData = {
  data: Record<string, Market.PairMarket>;
  event: WSEventName.HKEtfTicker;
  e: PricePeriod;
  t: string;
};
...
type PriceWSOption = {
  onOpen?: () => void;
  onPriceData?: (data: HkTickerWSData) => void;
...
export const useHkTickerWSData = (option?: PriceWSOption) => {
  const { memoEvents, msgEvent, send, onOpen } = useContext(WSContext);
  const [priceData, setPriceData] = React.useState<HkTickerWSData>();
  ...
  const messageHandle = (priceData: HkTickerWSData) => {
  ...
  msgEvent.hkTickerEvent.useSubscription(messageHandle);
  ...
  const subscribe = (params: string | string[]) => {
    ...
    const onOpenClear = onOpen(() => {
      ...
      if (msgList && msgList.length >= 0) {
        ...
        send(
          `{"method":"SUBSCRIBE","event":${WSEventName.HKEtfTicker}, "params": ${params}}`
        );
      }
    });
    const clear = () => {
