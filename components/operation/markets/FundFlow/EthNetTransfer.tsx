...
const FundFlow = ({status} : Props) => {
    ...
    const getData = async () => {
        const { data } = await getDataChart('1666032986125463571')
        ...
        //   return dayjs(item.timestamp).format('YYYY-MM')
        // })
        // const tList = rows.map((item: any) => {
        //   return item.exchanges
        // })
        // const pList = rows1.map((item: any) => {
        //   return item.price
        // })
        // time.map((item:any,index:number)=> {
        //   if(index < (time.length - rows1.length)){
        //     pList.unshift(0) 
        //   }
        // })
